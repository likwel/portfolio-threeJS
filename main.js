...
let experiences = [
  {
    title : "Data Engineer - Developper",
    society : "GEOMADAGASCAR",
    image: 'textures/project-spaze.webp',
    url: 'https://www.spaze.social/',
  },
  {
    title : "Data Analyst",
    society : "MADAGASCAR BUSINESS INTELLIGENCE",
    image: 'textures/project-myteachers.jpg',
    url: 'https://myteachers.com.au/',
  },
  {
...
let projectsCameraPos_xp = {
  x: 0,
  y: 0,
  z: 2,
};

let projectsCameraRot_xp = {
  y: 3.3,
  z :0,
  x :-0.1,
};
...
gltfLoader.load(
  ...
  function (room) {
    ...
    experienceMenuListener();
    init3DWorldClickListeners();
    initResponsive(room.scene);

    // gui.add(room.scene.position,'x').min(-9).max(9)
    // gui.add(room.scene.position,'y').min(-9).max(9)
    // gui.add(room.scene.position,'z').min(-9).max(9)

    // gui.add(room.scene.rotation,'x').min(-9).max(9)
    // gui.add(room.scene.rotation,'y').min(-9).max(9)
    // gui.add(room.scene.rotation,'z').min(-9).max(9)
...
function loadIntroText() {
  ...
  loader.load('fonts/unione.json', function (font) {
    ...
    const titleGeo = new TextGeometry('ELIE ANDRIATSITOHAINA', {
      font: font,
      size: 0.06,
      height: 0.01,
    });
...
function projectsMenuListener() {
  ...
  document.getElementById('projects-menu').addEventListener('click', function (e) {
    ...
    // document.getElementById('xp_btn').addEventListener('click', function (e) {
    //   e.preventDefault();
    //   disableOrbitControls();
    //   resetBookCover();

    //   gsap.to(camera.position, {
    //     ...projectsCameraPos_xp,
    //     duration: 1.5,
    //   });
    //   gsap.to(camera.rotation, {
    //     ...projectsCameraRot_xp,
    //     duration: 1.5,
    //   });
    //   gsap.delayedCall(1.5, enableCloseBtn);
...
function experienceMenuListener() {
  ...
  experiences.forEach((experience, i) => {
    ...
    const material = new THREE.MeshBasicMaterial({
      color: 0xffffff,
      map: new THREE.TextureLoader().load(experience.image),
      transparent: true,
      opacity: 0.0,
    });
    const xpPlane = new THREE.Mesh(geometry, material);
    xpPlane.name = 'experience';
    xpPlane.userData = {
      url: experience.url,
    };
    xpPlane.position.set(
      0.3 + i * 0.8 * colIndex,
      1 - rowIndex * 0.5,
      -1.15
    );
    xpPlane.scale.set(0, 0, 0);
    // mesh & y vars needed for animation
    experiences[i].mesh = xpPlane;
    experiences[i].y = 1 - rowIndex * 0.5;
    scene.add(xpPlane);
  });
    ...
    document.getElementById('xp_btn').addEventListener('click', function (e) {
      ...
      experiences.forEach((xp, i) => {
        xp.mesh.scale.set(1, 1, 1);
        gsap.to(xp.mesh.material, {
          opacity: 1,
          duration: 1.5,
          delay: 1.5 + i * 0.1,
        });
        gsap.to(xp.mesh.position, {
          y: xp.y + 0.05,
          duration: 1,
          delay: 1.5 + i * 0.1,
        });
